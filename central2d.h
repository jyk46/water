...
private:
    ...
    #define BYTE_ALIGN 4096
    #ifdef __INTEL_COMPILER
        typedef __declspec(align(BYTE_ALIGN)) std::vector<real, aligned_allocator<real, BYTE_ALIGN>> aligned_vector;
    #else // GCC
        typedef __attribute__ ((aligned(BYTE_ALIGN))) std::vector<real, aligned_allocator<real, BYTE_ALIGN>> aligned_vector;
    #endif
    ...
    inline int offset(int ix, int iy) const { return iy*nx_all*Physics::vec_size + ix*Physics::vec_size; }
...
void Central2D<Physics, Limiter>::apply_periodic()
{
    ...
    for (int iy = 0; iy < ny_all; ++iy)
        for (int ix = 0; ix < nghost; ++ix) {
            ...
            real *u_xy = u(ix, iy); __assume_aligned(u_xy, Physics::VEC_ALIGN);
            real *uwrap_xy = uwrap(ix, iy); __assume_aligned(uwrap_xy, Physics::VEC_ALIGN);

            real *u_ghost = u(nx+nghost+ix,iy); __assume_aligned(u_ghost, Physics::VEC_ALIGN);
            real *uwrap_ghost = uwrap(nx+nghost+ix,iy); __assume_aligned(uwrap_ghost, Physics::VEC_ALIGN);
    ...
    for (int ix = 0; ix < nx_all; ++ix)
        for (int iy = 0; iy < nghost; ++iy) {
            ...
            real *u_ghost = u(ix,ny+nghost+iy); __assume_aligned(u_ghost, Physics::VEC_ALIGN);
            real *uwrap_ghost = uwrap(ix,ny+nghost+iy); __assume_aligned(uwrap_ghost, Physics::VEC_ALIGN);
...
void Central2D<Physics, Limiter>::compute_step(int io, real dt)
{
    ...
    real *uh_cpy = (real *)_mm_malloc(sizeof(real) * Physics::vec_size, Physics::VEC_ALIGN);
    __assume_aligned(uh_cpy, Physics::VEC_ALIGN);
    ...
    for (int iy = 1; iy < ny_all-1; ++iy) {
        ...
        for (int ix = 1; ix < nx_all-1; ++ix) {
            ...
            #pragma unroll
            for(int m = 0; m < Physics::vec_size; ++m) uh_cpy[m] = uh[m];
    ...
    for (int j = nghost; j < ny+nghost; ++j){
        for (int i = nghost; i < nx+nghost; ++i){
            real *u_ij = u(i,j); __assume_aligned(u_ij, Physics::VEC_ALIGN);
            real *v_iio_jio = v(i-io, j-io); __assume_aligned(v_iio_jio,  Physics::VEC_ALIGN);
            #pragma unroll
            for(int m = 0; m < Physics::vec_size; ++m) u_ij[m] = v_iio_jio[m];
            // u(i,j) = v(i-io,j-io);
        }
    }
}
...
void Central2D<Physics, Limiter>::solution_check()
{
    ...
    for (int j = nghost; j < ny+nghost; ++j) {
        for (int i = nghost; i < nx+nghost; ++i) {
            // vec& uij = u(i,j);
            real *uij = u(i,j); __assume_aligned(uij, Physics::VEC_ALIGN);
            real h   = uij[0];
            h_sum   += h;
            hu_sum  += uij[1];
            hv_sum  += uij[2];
